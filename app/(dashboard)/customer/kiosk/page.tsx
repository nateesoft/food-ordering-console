...
export default function KioskPage() {
  ...
  // Sync selectedBranch into localStorage so fetchApi sends the correct x-branch-id header
  useEffect(() => {
    if (selectedBranch?.id) {
      localStorage.setItem('selectedBranchId', String(selectedBranch.id));
    }
  }, [selectedBranch?.id]);
  ...
  const categories = useMemo(() => {
    return ['ทั้งหมด', ...Array.from(new Set(menuItems.map(item => item.category)))];
  }, [menuItems]);
